...
class GoogleMapAPI {
	 ...
	 */
	 var $marker_clusterer_location = "js/markerclusterer_compiled.js";
    ...
    function GoogleMapAPI($map_id = 'map', $app_id = 'MyMapApp') {
        ...
        $this->app_id = $app_id;
        //$this->marker_clusterer_location = get_site_url().'/'."javascript/markerclusterer_compiled.js";
    }
     ...
     * @param string $dom_id DOM PCMW_Element ID for directions container.
    ...
    function getHeaderJS() {
        ...
        $_headerJS .= "<script src='http://maps.google.com/maps/api/js?file=api&amp;v=3&amp;&amp;key=".PCMW_GOOGLEMAPKEY."' type='text/javascript'></script>";
	...
	*      $map->printOnLoad();
	*/
    function printOnLoad($boolReturn = TRUE) {
       if($boolReturn)
     ...
     */
    function printOnLoadFunction($boolReturn = TRUE){
       if($boolReturn)
	 ...
	 */
	 function getMapFunctions(){
		...
		if($this->_display_js_functions===true){
	    	$_script = $this->getPCMW_UtilityFunctions();
        }
		return $_script;
	 }

	 function getPCMW_UtilityFunctions(){
		 $_script = "";
		 if(!empty($this->_markers))
		 ...
		 // PCMW_Utility functions used to distinguish between tabbed and non-tabbed info windows
			    ...
			    if(html!=''){
			        ...
			        if(sidebar_id != ''){
						...
						if(sidebar!=null && sidebar!=undefined && title!=null && title!=''){
							...
							")."
                            if(newlink.innerHTML.indexOf(title) < 0){
